 ...
 * The normal FastAPI endpoint is always attempted first. If the frontend
 * receives a 404/network failure, use the repository PFZ advisory catalogue
 * and live marine conditions so the PFZ UI remains functional.
 * No other frontend request is intercepted.
 */
(function () {
  const originalFetch = window.fetch.bind(window);
  const PFZ_PATH = /\/api\/marine-productivity\/pfz(?:\?|$)/i;
  const PFZ_CSV_URL = 'https://raw.githubusercontent.com/Maple483/ishita_sih/main/orca-backend/data/pfz_advisories.csv';
  ...
  window.fetch = async function (input, init) {
    ...
    try {
      ...
      return new Response(JSON.stringify({
        status: 'OK', user_location: { lat: 0, lon: 0 },
