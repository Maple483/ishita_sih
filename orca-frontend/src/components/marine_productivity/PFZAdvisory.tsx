...
function PFZMap({ userLat, userLon, results }: { userLat: number; userLon: number; results: PFZResult[] }) {
  ...
  return <div className="overflow-hidden rounded-xl border border-slate-800 bg-slate-950">
    <div className="flex items-center justify-between border-b border-slate-800 px-3 py-2">
      <div>
        <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400">PFZ map</div>
        <div className="text-[9px] text-slate-600 mt-0.5">Ranked advisory locations around the fisherman</div>
      </div>
      <div className="flex items-center gap-3 text-[9px] text-slate-500">
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-white" /> You</span>
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-emerald-400" /> Best PFZ</span>
      </div>
    </div>
...
export default function PFZAdvisory() {
  ...
  const useBrowserLocation = () => {
    if (!navigator.geolocation) {
      setError('Browser geolocation is not available. Enter coordinates manually.');
      return;
    }
  ...
  return <div className="space-y-4">
    <div className="flex items-center gap-2 rounded-xl border border-emerald-400/30 bg-emerald-500/5 p-3">
      <Fish className="h-5 w-5 shrink-0 text-emerald-400" />
      <div>
        <div className="text-sm font-bold text-emerald-300">Potential Fishing Zone Advisory</div>
        <div className="text-[10px] text-slate-500">Fisher location → live marine conditions → nearest ranked PFZ</div>
      </div>
    </div>
    ...
    <section className="rounded-xl border border-slate-800 bg-slate-900/60 p-3">
      <div className="mb-3 flex items-center justify-between">
        ...
        <button onClick={useBrowserLocation} className="flex items-center gap-1 rounded-lg border border-slate-700 px-2 py-1 text-[10px] text-cyan-300 hover:bg-slate-800">
          {locationLoading ? <RefreshCw className="h-3 w-3 animate-spin" /> : <LocateFixed className="h-3 w-3" />} Use device GPS
        </button>
      </div>
      <div className="grid grid-cols-2 gap-2">
        <label className="text-[10px] text-slate-500">Latitude
          <input value={lat} onChange={e => setLat(e.target.value)} type="number" step="any" className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 p-2 text-sm text-white outline-none focus:border-emerald-400" />
        </label>
        <label className="text-[10px] text-slate-500">Longitude
          <input value={lon} onChange={e => setLon(e.target.value)} type="number" step="any" className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 p-2 text-sm text-white outline-none focus:border-emerald-400" />
        </label>
      </div>
      <div className="mt-3 flex gap-2">
        ...
        <button onClick={findPFZ} disabled={loading} className="flex items-center gap-2 rounded-lg bg-emerald-500 px-3 py-2 text-xs font-bold text-slate-950 disabled:opacity-50">
          {loading ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Navigation className="h-4 w-4" />} Find best PFZ
        </button>
      </div>
    ...
    {data && <>
      {nearest && <section className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-3">
